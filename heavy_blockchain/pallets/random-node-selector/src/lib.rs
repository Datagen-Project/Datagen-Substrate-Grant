...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        ReliableNodeList { owners: Vec<(PeerId, T::AccountId)> },

        /// Number of elements in the map.
        TotalItemsInMap { total_items: u32 },
    }
    ...
    #[pallet::storage]
    pub(super) type RandomHash<T: Config> = StorageValue<_, T::Hash>;

    /// The last owner to check.
    #[pallet::storage]
    #[pallet::getter(fn reliable_node_to_check)]
    pub(super) type ReliableNodeToCheck<T: Config> = StorageValue<_, (PeerId, T::AccountId)>;
    ...
    pub type ReliableNode<T: Config> = CountedStorageMap<_, Blake2_128Concat, PeerId, T::AccountId>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight(100)]
        pub fn test_create_random_hash(origin: OriginFor<T>) -> DispatchResult {
        ...
        /// dev - This function is only for testing purposes.
        #[pallet::weight(100)]
        pub fn test_create_random_number(origin: OriginFor<T>) -> DispatchResult {
            let _sender = ensure_signed(origin)?;

            let random_number = Self::generate_random_number();

            <RandomNumber<T>>::put(random_number);
            Self::deposit_event(Event::RandomNumber {
        ...
        /// dev - This function is only for testing purposes.
        #[pallet::weight(100)]
        pub fn test_total_elements(origin: OriginFor<T>) -> DispatchResult {
            let _sender = ensure_signed(origin)?;
        ...
        #[pallet::weight(100)]
        pub fn test_get_owners_list(origin: OriginFor<T>) -> DispatchResult {
            let _sender = ensure_signed(origin)?;
            // Get all owners.
            let owners = <ReliableNode<T>>::iter().collect::<Vec<_>>();
            Self::deposit_event(Event::ReliableNodeList { owners });
            Ok(())
        }
        ...
        #[pallet::weight(100)]
        pub fn test_generate_random_number_range(origin: OriginFor<T>, max: u32) -> DispatchResult {
            let _sender = ensure_signed(origin)?;
        ...
        pub fn add_reliable_node(
            ...
            let _sender = ensure_signed(origin)?;
                ...
                !<ReliableNode<T>>::contains_key(&peer_id),
                Error::<T>::AlreadyTakenNode
            );
            // Add the owner to the list of owners.
            <ReliableNode<T>>::insert(&peer_id, &owner);
            Self::deposit_event(Event::SetOwnerReliableNode { peer_id, owner });
            Ok(())
        }
        ...
        #[pallet::weight(100)]
        pub fn remove_reliable_node(origin: OriginFor<T>, peer_id: PeerId) -> DispatchResult {
            let _sender = ensure_signed(origin)?;
            ensure!(
                <ReliableNode<T>>::contains_key(&peer_id),
                Error::<T>::NoReliableNodeToCheck
            );
            ensure!(
                _sender == <ReliableNode<T>>::get(&peer_id).unwrap(),
                Error::<T>::NotOwner
            );
        ...
        #[pallet::weight(100)]
        pub fn random_node_to_check(origin: OriginFor<T>) -> DispatchResult {
            let _sender = ensure_signed(origin)?;
        ...
        #[pallet::weight(100)]
        pub fn random_checker_node_selector(origin: OriginFor<T>) -> DispatchResult {
            let _sender = ensure_signed(origin)?;

            // Check if the reliable node to check has been chosen.
            ensure!(
                <ReliableNodeToCheck<T>>::exists(),
                Error::<T>::NoReliableNodeToCheck
            );
            ...
            loop {
                ...
                if selected_controller_to_unwrap_1 != selected_controller_to_unwrap_2
                    && selected_controller_to_unwrap_2 != <ReliableNodeToCheck<T>>::get()
                {
                    break;
                }
            }
            ...
            loop {
                ...
                if selected_controller_to_unwrap_1 != selected_controller_to_unwrap_3
                    && selected_controller_to_unwrap_2 != selected_controller_to_unwrap_3
                    && selected_controller_to_unwrap_3 != <ReliableNodeToCheck<T>>::get()
                {
                    break;
                }
            }
