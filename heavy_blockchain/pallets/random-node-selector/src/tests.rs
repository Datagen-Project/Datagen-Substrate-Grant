...
fn create_random_hash() {
    new_test_ext().execute_with(|| {
        // Create random hash.
        assert_ok!(RandomNodeSelector::test_create_random_hash(
...
fn create_random_number() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(RandomNodeSelector::test_create_random_number(
            RuntimeOrigin::signed(1)
        ));

        // Check the event
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
            crate::Event::RandomNumber { number: 0 },
        ));
        ...
        // Check the event
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
            crate::Event::RandomNumber { number: 1 },
        ));
    });
}
...
fn test_total_elements() {
    new_test_ext().execute_with(|| {
        assert_ok!(RandomNodeSelector::test_total_elements(
            RuntimeOrigin::signed(1)
        ));

        // Check the event
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
            crate::Event::TotalItemsInMap { total_items: 10 },
        ));
    });
}
...
fn check_add_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            RandomNodeSelector::add_reliable_node(
                RuntimeOrigin::signed(1),
                1,
                OpaquePeerId(vec![1, 1, 1, 1])
            ),
            crate::Error::<TestRuntime>::AlreadyTakenNode
        );

        // Add owner.
        // Need to set an free peer id.
        assert_ok!(RandomNodeSelector::add_reliable_node(
            RuntimeOrigin::signed(1),
            1,
            OpaquePeerId(vec![0, 1, 1, 1])
        ));
    });
}
...
fn check_remove_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            RandomNodeSelector::remove_reliable_node(
                RuntimeOrigin::signed(2),
                OpaquePeerId(vec![1, 1, 1, 1])
            ),
            crate::Error::<TestRuntime>::NotOwner
        );
        ...
        assert_noop!(
            RandomNodeSelector::remove_reliable_node(
                RuntimeOrigin::signed(1),
                OpaquePeerId(vec![1, 1, 1, 14])
            ),
            crate::Error::<TestRuntime>::NoReliableNodeToCheck
        );

        // Remove remove the peer id by the owner.
        assert_ok!(RandomNodeSelector::remove_reliable_node(
            RuntimeOrigin::signed(1),
            OpaquePeerId(vec![1, 1, 1, 1])
        ));
    })
...
fn check_initial_owners_list() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(RandomNodeSelector::test_get_owners_list(
            RuntimeOrigin::signed(1)
        ));
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
            crate::Event::ReliableNodeList {
                owners: vec![
...
fn random_node_range() {
    new_test_ext().execute_with(|| {
        assert_ok!(RandomNodeSelector::test_generate_random_number_range(
            RuntimeOrigin::signed(1),
            2
        ));
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
...
fn random_node_to_check() {
    new_test_ext().execute_with(|| {
        // Dispatch a signed extrinsic.
        assert_ok!(RandomNodeSelector::random_node_to_check(
            RuntimeOrigin::signed(1)
        ));
        ...
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
            crate::Event::ReliableNodeToCheck {
                owner: 8,
        ...
        assert_eq!(
            RandomNodeSelector::reliable_node_to_check(),
            Some((OpaquePeerId(vec![8, 8, 8, 8]), 8))
        );
    })
}
...
fn random_checker_node_selector() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            RandomNodeSelector::random_checker_node_selector(RuntimeOrigin::signed(1)),
            crate::Error::<TestRuntime>::NoReliableNodeToCheck
        );
        ...
        assert_ok!(RandomNodeSelector::random_checker_node_selector(
            RuntimeOrigin::signed(1)
        ));

        // Check the event
        System::assert_last_event(RuntimeEvent::RandomNodeSelector(
            crate::Event::Controllers {
