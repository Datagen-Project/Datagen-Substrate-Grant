...
//! XCM configurations for the Datagen runtime.
//DatagenParachainNetwork
use super::{
	rialto_messages::ToRialtoBlobExporter,
	datagen_parachain_messages::ToDatagenParachainBlobExporter, AccountId, AllPalletsWithSystem,
...
parameter_types! {
	...
	pub const ThisNetwork: NetworkId = CustomNetworkId::Datagen.as_network_id();
	...
	pub const DatagenParachainNetwork: NetworkId = CustomNetworkId::DatagenParachain.as_network_id();
...
pub type OnDatagenBlobDispatcher = xcm_builder::BridgeBlobDispatcher<
...
impl xcm_executor::Config for XcmConfig {
	...
	type FeeManager = ();
	type MessageExporter = ToRialtoOrDatagenParachainSwitchExporter;
...
pub struct ToRialtoOrDatagenParachainSwitchExporter;

impl ExportXcm for ToRialtoOrDatagenParachainSwitchExporter {
	...
	fn validate(
		...
		if network == RialtoNetwork::get() {
		...
		} else if network == DatagenParachainNetwork::get() {
			ToDatagenParachainBlobExporter::validate(
				network,
				channel,
				universal_source,
				destination,
			...
			.map(|result| ((DatagenParachainNetwork::get(), result.0), result.1))
		} else {
	...
	fn deliver(ticket: Self::Ticket) -> Result<XcmHash, SendError> {
		...
		if network == RialtoNetwork::get() {
			...
			ToDatagenParachainBlobExporter::deliver(ticket)
		} else {
			Err(SendError::Unroutable)
		}
	}
...
/// Emulating XCMP channel with sibling chain. We don't have required infra here, at Datagen,
