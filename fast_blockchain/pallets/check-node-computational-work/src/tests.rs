...
fn correct_hook_event_when_hash_work_is_called() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(pallet_computational_work::hash_work(Origin::signed(get_account_id_from_seed::<sr25519::Public>("Alice"))));
...
fn correct_reset_when_final_result_is_emitted() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ComputationalWork::hash_work(Origin::signed(get_account_id_from_seed::<sr25519::Public>("Alice"))));
		let last_computational_work_is_checked = ComputationalWork::last_computational_work_is_checked();
...
fn not_change_last_computational_work_if_not_checked() {
	new_test_ext().execute_with(|| {
		...
		run_to_block(11);
		assert_ok!(ComputationalWork::hash_work(Origin::signed(get_account_id_from_seed::<sr25519::Public>("Bob"))));

		assert_eq!(last_computational_work, ComputationalWork::last_computational_work());
	})
}
...
fn change_last_computational_work_if_is_checked() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ComputationalWork::hash_work(Origin::signed(get_account_id_from_seed::<sr25519::Public>("Alice"))));
		let last_computational_work = ComputationalWork::last_computational_work().unwrap();
		assert_eq!(last_computational_work.2, get_account_id_from_seed::<sr25519::Public>("Charlie"));
	})
}